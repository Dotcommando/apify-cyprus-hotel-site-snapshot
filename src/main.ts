...
async function tryDismissConsent(page: Page, startedAtIso: string): Promise<IConsentLog[]> {
  const logs: IConsentLog[] = [];
  const add = (entry: Omit<IConsentLog, 'at'>) => logs.push({ at: startedAtIso, ...entry });
  ...
  const crawler = new PlaywrightCrawler({
    ...
    async requestHandler(ctx) {
      ...
      try {
        ...
        const pageDoc: ICrawledPageSnapshot = {
          url: request.url,
          finalUrl,
          status,
          contentType,
